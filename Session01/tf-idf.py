...
def gather_20newsgroup_data():
    ...
    def collect_data_from(parents_dir, newsgroup_list):
        data = []
        for group_id, newsgroup in enumerate(newsgroup_list):
            label = group_id
            dir_path = parents_dir + '/' + newsgroup + '/'
    ...
    train_data = collect_data_from(
            parents_dir = train_dir,
            newsgroup_list = list_newsgroups)
    test_data =  collect_data_from(
            parents_dir = test_dir,
            newsgroup_list = list_newsgroups)
...
def get_tf_idf(data_path):
    ...
    for document in documents:
        ...
        for word in word_set:
            ...
            word_tfidfs.append((words_IDs[word], tf_idf_value))
